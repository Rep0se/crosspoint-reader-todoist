...
bool TodoistClient::fetchTodaysTasks(const char* apiKey, std::vector<TodoistTask>& outTasks) {
  ...
  // Use a filter document to extract only id, content, and due.date, minimising heap use.
  ...
  // Read via getString(), NOT getStream(). Todoist's CloudFront edge replies with
  // Transfer-Encoding: chunked over HTTP/1.1, and HTTPClient::getStream() does NOT
  // de-chunk. Feeding the raw stream to deserializeJson makes the parser read the
  // chunk-size hex framing as JSON, producing an empty doc with NO error — the fetch
  // "succeeds" with zero tasks ("No tasks for today"). getString() de-chunks; the
  // filtered today response is only a few KB, so the transient String is acceptable.
